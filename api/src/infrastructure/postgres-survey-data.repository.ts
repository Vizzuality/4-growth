...
export class PostgresSurveyDataRepository implements ISurveyDataRepository {
  private readonly answersTable: string = 'survey_answers';
  ...
  private async appendBaseWidgetData(
    ...
    const totalsSql = `SELECT answer as "key", count(answer)::integer as "count", SUM(COUNT(answer)) OVER ()::integer AS total FROM ${this.answersTable} ${this.sqlAdapter.appendExpressionToFilterClause(filterClause, `question_indicator = '${indicator}'`)} GROUP BY answer`;
