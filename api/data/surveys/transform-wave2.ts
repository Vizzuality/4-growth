...
export async function loadWave2Entities(wave2Dir: string): Promise<{
  Root: RootRecord[];
  surveyData: Map<string, EntityRecord[]>;
  referenceLookup: Map<number, string>;
}> {
  // Load Root (required)
  const rootPath = path.join(wave2Dir, 'Root.json');
  let Root: RootRecord[];
  try {
  ...
  for (const entity of SURVEY_DATA_ENTITIES) {
    const filePath = path.join(wave2Dir, `${entity}.json`);
  ...
  for (const table of REFERENCE_TABLES) {
    const filePath = path.join(wave2Dir, `${table}.json`);
...
export async function transformWave2FromOData(
  ...
  const logger = new Logger('ETL-Wave2-Transform');

  logger.log(`Loading Wave 2 entities from: ${wave2Dir}`);
  const { Root, surveyData, referenceLookup } =
    await loadWave2Entities(wave2Dir);
  ...
  for (const root of Root) {
    ...
    for (const record of childRecords) {
      ...
      for (const { variable, answer } of answers) {
        const question = WAVE2_VARIABLE_TO_QUESTION[variable];
        if (!question) {
          continue;
        }
