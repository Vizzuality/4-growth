/**
 * Wave 3 Variable to Question Text Mapping
 *
 * This mapping normalizes Wave 3 OData variable names (column names) to their
 * corresponding question text. For questions that exist in previous waves, we use
 * the Wave 1 question text to ensure all waves share the same indicator and
 * their data is combined in all queries.
 *
 * TODO: Update this mapping when the actual Wave 3 questions are known.
 * Initially using the same mapping as Wave 2.
 *
 * Categories:
 * - Shared questions: Normalize to W1 text so all waves share the same indicator
 * - Rephrased questions: Rephrased version, normalize to W1 text for same indicator
 * - New questions: New question text for new indicators
 */
export const WAVE3_VARIABLE_TO_QUESTION: Record<string, string> = {
  // Shared questions - normalize to W1 text so all waves share the same indicator
...
export const MAPPED_VARIABLES = Object.keys(WAVE3_VARIABLE_TO_QUESTION);

export function hasQuestionMapping(variable: string): boolean {
  return variable in WAVE3_VARIABLE_TO_QUESTION;
}

export function getQuestionText(variable: string): string | undefined {
  return WAVE3_VARIABLE_TO_QUESTION[variable];
}
