 ...
 *
 * This mapping normalizes Wave 2 CSV variable names to their corresponding
 * question text. For questions that exist in both waves, we use the Wave 1
 * question text to ensure both waves share the same indicator and their
 * data is combined in all queries.
 */
const WAVE2_VARIABLE_TO_QUESTION: Record<string, string> = {
  ...
  Primary_Area_of_Operation_in_forestry: 'Primary area of operation in forestry',
  ...
  What_network_connectivity_do_you_use:
...
interface CsvRow {
  ...
  RootID: string;
  answer_type: string;
  survey_chapter: string;
  value: string;
  variable: string;
  Description: string;
  categorical_answer_ID: string;
  Name: string;
  Identifier: string;
}
...
export const transformWave2 = async () => {
  const logger = new Logger('ETL-Wave2');

  logger.log('Starting Wave 2 CSV transform...');

  // Read the CSV file
  const csvContent = await fs.readFile(
    `${__dirname}/wave2-export.csv`,
    'utf-8',
  );
  ...
  logger.log(`Parsed ${records.length} total rows from CSV`);

  // Filter to only categorical_answer rows with actual answers
  const categoricalAnswers = records.filter(
    (row) =>
      row.answer_type === 'categorical_answer' &&
      row.Description?.trim() !== '' &&
      WAVE2_VARIABLE_TO_QUESTION[row.variable] !== undefined,
  );

  logger.log(`Found ${categoricalAnswers.length} categorical answers with valid mapping`);

  // Group by survey ID (Identifier column) to build per-survey data
  const surveyMap = new Map<string, CsvRow[]>();
  for (const row of categoricalAnswers) {
    const surveyId = row.Identifier;
    if (!surveyMap.has(surveyId)) {
      surveyMap.set(surveyId, []);
    }
    surveyMap.get(surveyId)!.push(row);
  }

  logger.log(`Found ${surveyMap.size} unique surveys`);

  // Transform to survey answers
  const transformedAnswers: SurveyAnswer[] = [];
  let skippedSurveys = 0;

  for (const [surveyId, rows] of surveyMap) {
    // Find country from Location_CountryRegion
    const countryRow = rows.find(
      (r) => r.variable === 'Location_CountryRegion',
    );

    if (!countryRow) {
      logger.warn(`Survey ${surveyId}: No country found, skipping`);
      skippedSurveys++;
      continue;
    }

    const countryName = countryRow.Description.trim();
    const countryCode = CountryISOMap.getISO3ByCountryName(countryName);

    if (!countryCode) {
      logger.error(
        `Survey ${surveyId}: Unknown country "${countryName}", skipping`,
      );
      skippedSurveys++;
      continue;
    }
    ...
    for (const row of rows) {
      const question = WAVE2_VARIABLE_TO_QUESTION[row.variable];
      if (!question) {
        // Should not happen since we filtered above, but safety check
        continue;
      }

      // Clean up the answer text - remove any trailing language variants in parentheses
      const answer = row.Description.replace(/ \([\w\s]+\)$/g, '').trim();
  ...
  logger.log(`Transformed ${transformedAnswers.length} answers`);
  logger.log(`Skipped ${skippedSurveys} surveys due to missing/invalid country`);
  ...
  logger.log(
    `\nCreated file ${__dirname}/surveys-wave2.json with ${surveyMap.size - skippedSurveys} surveys`,
  );
};
