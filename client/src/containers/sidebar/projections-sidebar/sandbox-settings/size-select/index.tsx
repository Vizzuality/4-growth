...
function getSelectedSizeOption(
  ...
  if (isBubbleChartSettings(settings)) {
    const key = getValues(settings)[0]
      .size as keyof typeof PROJECTION_TYPE_TO_LABEL_MAP;
...
const SizeSelect: FC<{
  options: CustomProjectionSettingsType["bubble_chart"]["size"];
}> = ({ options }) => {
  const { settings, setBubbleChartIndicator } = useSettings();
  const selected = useMemo(() => getSelectedSizeOption(settings), [settings]);
