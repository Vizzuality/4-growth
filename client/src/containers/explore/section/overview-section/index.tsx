...
const OverviewSection: FC<OverviewSectionProps> = ({
  ...
  // Read from the payload rather than the data-source filter, so this layout and
  // the widgets inside it cannot disagree while a refetch is in flight.
  const isComparingSources =
    (secondWidget.data.percentages.bySource?.length ?? 0) > 1;

  return (
    <>
      {isComparingSources ? (
        <div
          className="grid grid-cols-1 gap-0.5 lg:col-span-2 lg:min-h-[265px] lg:grid-cols-2"
          data-testid="overview-single-values"
        >
          <Widget
