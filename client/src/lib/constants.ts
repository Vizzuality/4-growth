...
/**
 * Indicators whose answers carry a fixed meaning, so colour follows the answer
 * rather than its rank. Elsewhere the charts tint the largest value, which on
 * these questions moves the highlight between answers as the data source
 * changes. Kept as an explicit list because `Yes` is not the headline answer on
 * every yes/no question — on "Are there specific barriers hindering further
 * integration?" it is the bad outcome.
 */
export const SEMANTIC_ANSWER_INDICATORS = ["adoption-of-technology-by-country"];

const SEMANTIC_ANSWER_CSS_COLORS: Record<string, string> = {
  Yes: "hsl(var(--accent))",
  No: "hsl(var(--secondary))",
  "Not at all": "hsl(var(--secondary))",
  "Don't know": "hsl(var(--muted-foreground))",
};

const SEMANTIC_ANSWER_TW_COLORS: Record<string, string> = {
  Yes: "bg-accent",
  No: "bg-secondary",
  "Not at all": "bg-secondary",
  "Don't know": "bg-muted-foreground",
};

function getSemanticAnswerColor(
  palette: Record<string, string>,
  indicator: string | undefined,
  label: string | number | undefined,
): string | undefined {
  if (indicator === undefined) return undefined;
  if (SEMANTIC_ANSWER_INDICATORS.includes(indicator) === false)
    return undefined;
...
export function getSemanticAnswerCssColor(
  indicator: string | undefined,
  label: string | number | undefined,
): string | undefined {
  return getSemanticAnswerColor(SEMANTIC_ANSWER_CSS_COLORS, indicator, label);
}

export function getSemanticAnswerTwColor(
  indicator: string | undefined,
  label: string | number | undefined,
): string | undefined {
  return getSemanticAnswerColor(SEMANTIC_ANSWER_TW_COLORS, indicator, label);
}
